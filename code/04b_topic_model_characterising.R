...
#                   "patchwork", "OverReact")
# load_packages(package.list,repos = "https://cran.ma.imperial.ac.uk/")
...
# create subfolder
createMySubfolder(subfolderName = "characterising_bertopic_models")
...
topic_doc_matrix=read_csv("code/berttopic/document_topic_matrix.csv")
...
# plot
p_opt=topic_opt %>% 
  ggplot(aes(x=K,Deveaud2014)) +
  geom_point() +
  geom_line() +
  geom_hline(yintercept = 1, linetype="dotted", col= "red")+
  geom_vline(xintercept = topic_opt$K[which.max(topic_opt$Deveaud2014)], 
             linetype="dotted", col= "red")+
  geom_point(data = topic_opt[which.max(topic_opt$Deveaud2014),], 
             shape =1, col = "red",size=5)+
  OverReact::theme_react() +
  labs(x="Number of topics (k)", y="Jenson-Shannon divergence (normalised to 1-max)",
       title = "Topic modelling optimisation for k")

# save
OverReact::saveREACTplot(p = p_opt,figpath = figpath,filename = "topics_optimisation_k",
                         width = 6,height = 5,savePDF = T,filetypes = c("png","jpg"))
...
# get wide gamma
td_beta_wide=td_beta %>% rename(topic_num=topic) %>% 
  pivot_wider(id_cols = topic_num,
              names_prefix = "",names_from = term,
              values_from = beta)
...
topics_plot=gamma_terms %>%
  ...
  labs(x = NULL, y = expression(gamma),
       ...
       subtitle = "With the top 8 words that contribute to each topic")
...
# designate 'interesting' topics ------------------------------------------

goodtopics=c(7, # post covid recovery
             15, # face masks
             23, # preganant women
             25, # death rates and excess deaths
             26, # variants
             37, # immune response
             41, # long covid
             46, # cellular analysis
             54, # psychological impact
             59, # antibodies
             70, # interventions
             77, # risk factors
             78, # viral load, RNA
             ...
             81, # NPIs
             86, # symptoms
             91, # lung abnormalities
             95, # drug discovery
             98, # vaccines
             105, # spike protein
             108, # respiratory problems
             109, # neurological problems
             113, # therapeutics,
             116, # severe COVID
             118, # mortality rates
             128, # genomics  and mutations
             131, # psychological impacts / recovery
             138 # pcr testing
)

# save
saveRDS(goodtopics,file = paste0(outpath,"topics_of_interest.rds"))
...
cormat=cor((gamma_mat[,2:141]))

# add names
rownames(cormat) = colnames(cormat) = paste0("Topic ",1:140,": ", top_terms_3$terms)


myheatmap=ComplexHeatmap::Heatmap(name = "Correlation",cormat,row_dend_width = unit(0.3,"cm"),
...
png(filename = paste0(figpath,"topic_heatmap.png"),width = 13,height = 13,units = "in",res = 300)
draw(myheatmap,heatmap_legend_side="top")
dev.off()
...
# get preprint mat
gamma_mat_preprint=gamma_mat %>% filter(unique_id %in% dat_preprint$id)



# get correlation matrix
cormat=cor((gamma_mat_preprint[,2:141]))
...
png(filename = paste0(figpath,"topic_heatmap_preprint.png"),width = 13,height = 13,units = "in",res = 300)
draw(myheatmap,heatmap_legend_side="top")
dev.off()


# save topics as table
OverReact::saveREACTtable(tab = top_terms,outpath = outpath,filename = "top_terms_8")
...
dat_join=dat %>% 
  left_join(gamma_mat, by=c("id"="unique_id"))
...
colnames(topic_summary_df)[3:(ntopic+2)] <- paste0("Topic ",1:ntopic,": ",top_terms$terms)

# Pivot for plotting
topic_summary_df_long <- topic_summary_df %>% 
  pivot_longer(cols =colnames(topic_summary_df)[3:(ntopic+2)])

# forate as date
topic_summary_df_long$date=as.Date(topic_summary_df_long$date)

# marc's prefs:
marc_topics=c(13,26,7,98)
marc_topics_string=paste0(" ",marc_topics,":")
marc_topics_string_comb=paste(marc_topics_string,collapse = "|")
datethresh=as.Date("2020-01-01")
# Plot
p_topics=topic_summary_df_long %>% 
  dplyr::filter(
    date>datethresh,
    (grepl(marc_topics_string_comb,name)
    )) %>% 
  ggplot2::ggplot(aes(x=date,y=value, col=name,group=name)) +
  geom_point(aes(x=date,y=value,group=name),
             data = topic_summary_df_long %>% 
               dplyr::filter(date>datethresh), col="black",alpha=0.01,size=0.5)+
  geom_line(aes(x=date,y=value, group=name),
            data = topic_summary_df_long%>% 
              dplyr::filter(date>datethresh), col="black",alpha=0.03,size=0.6) +
  geom_point()+
  geom_line() +
  OverReact::theme_react() +
  scale_y_log10()+
  OverReact::scale_color_imperial(palette = "default") +
  labs(x="",y="Mean Topic Gamma", col="Topic",title="Changing topic enrichment over time") +
  theme(legend.position =c(0.33,0.84))+  
...
saveRDS(object = p_topics,file = paste0(figpath,"topics_over_time_plot.rds"))


# Combine with topic model univariate over time results -------------------

univ_res=readRDS("output/univariate_topics_over_time/time_varying_topics_association_res.rds")

# save to their own objects
p_heatmap=univ_res$p_heatmap
p_alt_over_time_mean_type=univ_res$p_alt_over_time_mean_type


p_comb_2=p_topics/p_heatmap + plot_layout(heights = c(4,5))
p_comb_2
...
dat_mod <- dat %>% left_join(td_gamma_wide, by =c("id"="document")) %>% 
