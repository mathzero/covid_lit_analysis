...
# join with main data
dat_mod <- dat %>% 
  left_join(td_gamma_wide, by =c("unique_id"="document")) %>% 
...
dat_mod <- dat_mod %>%  left_join(dat_text_titles, by =c("unique_id")) 
...
citationvars=c("citations_count","cites_per_day","metrics_recent_citations")
altmetricvars=c(names(dat_mod)[grepl("cited_by",names(dat_mod))],
                names(dat_mod)[grepl("cohort",names(dat_mod))],
                names(dat_mod)[grepl("readers",names(dat_mod))],
                names(dat_mod)[grepl("context",names(dat_mod))],
                "altmetrics_score")
...
p_roc=plotReactROC(list_of_preds=list(`5. Title text data` = xval_title$predictions,
...
dat_mod$subs_pub_if_5[is.na(dat_mod$subs_pub_if_5)] <- 0
dat_mod$subs_pub_if_10[is.na(dat_mod$subs_pub_if_10)] <- 0

xval_altmetrics_top=stabilityshap::catboostCrossvalidate(X=dat_mod %>% dplyr::select(all_of(altmetricvars)),
                                                     y=dat_mod$subs_pub_if_5,
                                                     my_params = NULL,k = 10,myseed = 123,
                                                     loss_function = "Logloss",eval_metric = "AUC")
...
# create ROC curve from all predictions
p_roc_top=plotReactROC(list_of_preds=list(`5. Title text data` = xval_title_top$predictions,
...
# create ROC curve from all predictions
p_roc_top10=plotReactROC(list_of_preds=list(`5. Title text data` = xval_title_top10$predictions,
...
xval_altmetrics_medrxiv=stabilityshap::catboostCrossvalidate(X=dat_mod_medrxiv %>% dplyr::select(all_of(altmetricvars)),
                                                     y=dat_mod_medrxiv$subs_pub_if_10,
                                                     my_params = NULL,k = 10,myseed = 123,
                                                     loss_function = "Logloss",eval_metric = "AUC")
